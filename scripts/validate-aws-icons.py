...
def extract_shape_name(full_style):
    """Extract the prIcon or resIcon shape name from a full style string."""
    for part in full_style.split(';'):
        part = part.strip()
        if part.startswith('prIcon='):
            # e.g., prIcon=mxgraph.aws4.lambda -> lambda
            val = part.split('=', 1)[1]
...
def validate_icons(available_shapes, aws_icons):
    ...
    for service in aws_icons['services']:
        ...
        # Check exact match
        if shape_name in shape_set:
            results['valid'].append({
                'service_name': service_name,
                'service_id': service_id,
                'shape_name': shape_name
            })
        elif shape_name.lower() in shape_set_lower:
            # Case mismatch
            results['case_mismatches'].append({
                'service_name': service_name,
                'service_id': service_id,
                'used_shape': shape_name,
                'correct_shape': shape_set_lower[shape_name.lower()]
            })
        else:
            # Find close matches
...
def main():
    ...
    shapes_file = project_root / '.archive' / 'aws4-available-shapes.txt'
    icons_file = project_root / 'assets' / 'aws-icons.json'

    if not shapes_file.exists():
        print(f"Error: {shapes_file} not found", file=sys.stderr)
        print("Run: python scripts/extract-shape-names.py <aws4.xml> --output .archive/aws4-available-shapes.txt")
        sys.exit(1)
