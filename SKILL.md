...
Converts architecture diagram images to DrawIO XML format, with focus on GCP cloud services.
...
| Vertex AI | `mxgraph.gcp2.vertexai` |
| Cloud Scheduler | `mxgraph.gcp2.cloud_scheduler` |
| Apigee | `mxgraph.gcp2.apigee` |
| Pub/Sub | `mxgraph.gcp2.pubsub` |
...
2. **Select components** - From libraries:
   - Look up each service in `assets/gcp-icons.json`
   - Select appropriate containers from `assets/containers.json`
...
- Check that `<mxGeometry>` has valid width/height
...
| **Assets** | |
| `assets/gcp-icons.json` | GCP service icon database (35 services) |
| `assets/containers.json` | Container and connection styles |
...
| `references/style-guide.md` | Styling best practices |
| **Scripts** | |
